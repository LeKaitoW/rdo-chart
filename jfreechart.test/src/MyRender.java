 ...
 * @since 1.0.13
 */
public class MyRender extends AbstractXYItemRenderer
    ...
    public static class State extends XYItemRendererState {
        ...
        private void stepTo(double x, double y) {
            if (Double.isNaN(x) || Double.isNaN(y)) {
                return;
            }
            double lastY = this.seriesPath.getCurrentPoint().getY();
            if (y == lastY) {
                // this represents the situation
                // for drawing a horizontal bar.
                this.seriesPath.lineTo(x, y);
            }
            else {  //this handles the need to perform a 'step'.

                // calculate the step point
            	this.seriesPath.lineTo(x, lastY);
            	this.seriesPath.lineTo(x, y);
            }
        }
    }
    ...
    public void drawItem(Graphics2D g2, XYItemRendererState state, 
        ...
        if (!Double.isNaN(transX1) && !Double.isNaN(transY1)) {
            ...
            if (s.lastPointGood) {
            	double deltaX = Math.abs(x - s.lastX);
                if ( deltaX > 2 * s.dX) {
            		double firstExtremumX = s.lastX + deltaX/3;
            		double secondExtremumX = firstExtremumX + deltaX/3;
                	if (s.lastExtremumHigh) {
                		s.stepTo(firstExtremumX, s.lowY);
                		s.stepTo(secondExtremumX, s.highY);
                		s.stepTo(x, y);
                	}
                	else {
                		s.stepTo(firstExtremumX, s.highY);
                		s.stepTo(secondExtremumX, s.lowY);
                		s.stepTo(x, y);
                	}
                    s.lastX = x;
                    s.openY = y;
                	...
                	if (y > s.highY) {
                		s.highY = y;
                		s.lastExtremumHigh = true;
                	}
                	
                	if (y < s.lowY) {
                		s.lowY = y;
                		s.lastExtremumHigh = false;
                	}                		
                }
            }
            else {
    ...
    public Object clone() throws CloneNotSupportedException {
        MyRender clone = (MyRender) super.clone();
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof MyRender)) {
            return false;
        }
        if (!super.equals(obj)) {
            return false;
        }
        MyRender that = (MyRender) obj;
